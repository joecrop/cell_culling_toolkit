...
sub add_libraries {
	print SIMFILE ".inc 'models/nfet.inc'\n";
	print SIMFILE ".inc 'models/pfet.inc'\n";
...
#		}
		...
		for (@$cref) {
			...
			while ( my ($key, $value) = each(%spice) ) {
				...
				if( $data[0] eq "in" ) #is input
				{
					if($value eq '1')
					{
						...
						print SIMFILE "XNAND_".$data[1]."_1 ".$data[1]."_a VDD ".$data[1]."_in VDD VSS VNW VPW $nand2_name \n";
						print SIMFILE "XNAND_".$data[1]."_2 ".$data[1]." VDD ".$data[1]."_a VDD VSS VNW VPW $nand2_name \n";
					}
					else
					{
				...
				else #is output
				{
					...
					print SIMFILE "XNAND_".$data[1]."_3 ".$data[1]."_out VDD ".$data[1]." VDD VSS VNW VPW $nand2_name \n";
				}
			}
			...
			for($i = 0; $i < @names; $i++)
			{
				$names[$i] =~ s/ *avg//g;
				$names[$i] = uc($names[$i]);
				if(($names[$i] ne "TEMPER") && ($names[$i] ne "ALTER#") && ($names[$i] ne ""))
				{
				...
				else #is output
				{
					...
					while ( my ($k2, $val2) = each(%in_change) )
					{
						...
						if($in_change{$k2} eq "0to1")
						{
							...
							print SIMFILE "XNAND_".$out_name[1]."_3 ".$out_name[1]."_out VDD ".$out_name[1]." VDD VSS VNW VPW $nand2_name \n";
							last;
						}
						else
						{
